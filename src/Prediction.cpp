...
void Prediction::DoPredict(int lane, int prev_size, double car_s, std::map<int, Snapshot> &sensor_fusion) {
  ...
  while (it != sensor_fusion.end())
  {
    ...
    if (d<(2+4*lane+2) && d>(2+4*lane-2)) {
      ...
      if ((check_car_s > car_s) &&( (check_car_s-car_s) < 30)) {
        ...
        check_speed = sqrt(it->second.vx*it->second.vx + it->second.vy*it->second.vy);
        too_close = true;
      }
